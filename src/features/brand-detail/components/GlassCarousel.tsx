      ...
      {glasses.length > 1 && (
        <div className="flex gap-2 justify-center">
          {glasses.map((_, index) => (
            <button
              key={index}
              className={`h-3 rounded-full border-2 border-primary bg-white cursor-pointer transition-default p-0 hover:bg-accent hover:border-accent ${index === currentIndex ? 'bg-primary w-8 rounded-md' : 'w-3'
                }`}
